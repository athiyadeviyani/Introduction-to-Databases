...
SELECT S.uun 
FROM   students S 
WHERE  S.uun NOT IN (SELECT E.student 
                     FROM   exams E); 
...
SELECT Fail.student 
FROM   (SELECT E.student, 
               Count(E.grade) AS fail_count 
        FROM   exams E 
        WHERE  E.grade < 40 
        GROUP  BY E.student) AS Fail 
       JOIN (SELECT E.student, 
...
SELECT DC.degree, 
       Sum(DC.credits) 
FROM   (SELECT P.degree, 
               P.course, 
               C.credits 
        FROM   programmes P 
               JOIN courses C 
                 ON P.course = C.code) AS DC 
GROUP  BY DC.degree; 
...
SELECT ExamCounts.student, 
       ExamCounts.max AS date 
FROM   (SELECT E.student, 
               Recent.max, 
               Count(E.student) 
        FROM   exams E 
...
FROM   programmes P 
       INNER JOIN students S 
               ON S.degree = P.degree 
       LEFT OUTER JOIN exams E 
                    ON E.course = P.course 
                       AND E.student = S.uun 
...
SELECT S.uun, 
       S.NAME 
FROM   students S 
WHERE  S.uun NOT IN (SELECT DISTINCT( S.uun ) 
                     FROM   programmes P 
                            INNER JOIN students S 
                                    ON S.degree = P.degree 
                            LEFT OUTER JOIN exams E 
